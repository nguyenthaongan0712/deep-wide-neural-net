...
def variance_est(n_width, n_pts, temp_mat, n_nets):
    with torch.no_grad():    
        ...
        net = TwoLayersNet(n_width, n_out = n_nets)
        sig_testvtest = torch.var(net(circle_test), dim = 1, keepdim = True).cpu()
        sig_testvtrain = torch.mm(net(circle_test), torch.t(net(input_data))).cpu()/n_nets
        sig_trainvtrain =torch.mm(net(input_data), torch.t(net(input_data))).cpu()/n_nets
...
class AnimationPlot_lsq(object):
    def __init__(self, n_nets, input_data, K_testvtrain, K_trainvtrain, train_target, line_tuple, ax,
       n_wid = 50, n_out = 1, n_pts = 100, epochs_per_frame = 1):
        ...
        self.circle_test = circle_transform(self.gamma_vec)
        self.ax = ax

        for i in range(self.n_nets):
            self.__dict__['net {}'.format(i)] = TwoLayersNet(n_wid, n_out)
            self.test_output[:, i] = self.__dict__['net {}'.format(i)](self.circle_test).cpu().detach().numpy().flatten()
            self.train_output[:, i] = self.__dict__['net {}'.format(i)](self.input_data).cpu().detach().numpy().flatten()
...
def kernel_mats(net, gamma_train, gamma_test, n_train = 4, kernels='both'):
